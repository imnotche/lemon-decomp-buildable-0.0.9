...
public class PullCrystal extends Module
{
    ...
    public PullCrystal() {
        ...
        this.pushTarget = this.registerBoolean("Push Target", false, () -> this.page.getValue().equals("Calc"));
        this.fiveB = this.registerBoolean("5b Mode", false, () -> this.page.getValue().equals("Calc"));
        ...
        this.bypass = this.registerBoolean("Bypass", false, () -> this.silentSwitch.getValue() && this.page.getValue().equals("General"));
        this.dance = this.registerBoolean("Hotbar Dance (?", false, () -> this.silentSwitch.getValue() && this.bypass.getValue() && this.page.getValue().equals("General"));
        ...
        this.iq = this.registerBoolean("IQ", false, () -> this.render.getValue() && this.page.getValue().equals("Render"));
        this.speed = this.registerDouble("Speed", 0.5, 0.01, 1.0, () -> this.render.getValue() && this.iq.getValue() && this.page.getValue().equals("Render"));
        this.hud = this.registerBoolean("HUD", false, () -> this.page.getValue().equals("Render"));
        this.movingPistonNow = new Vec3d(-1.0, -1.0, -1.0);
        this.lastBestPiston = null;
        this.movingCrystalNow = new Vec3d(-1.0, -1.0, -1.0);
        this.lastBestCrystal = null;
        this.movingRedstoneNow = new Vec3d(-1.0, -1.0, -1.0);
        ...
        this.receiveListener = new Listener<PacketEvent.Receive>(event -> {
            if (PullCrystal.mc.world == null || PullCrystal.mc.player == null || this.crystalPos == null) {
            }
            else {
                if (event.getPacket() instanceof SPacketSoundEffect) {
                    final SPacketSoundEffect packet = (SPacketSoundEffect)event.getPacket();
                    if (packet.getCategory() == SoundCategory.BLOCKS && packet.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE && this.crystalPos.distanceSq(packet.getX(), packet.getY(), packet.getZ()) <= 9.0) {
                        this.boom = true;
                    }
                }
            }
        ...
        this.listener = new Listener<PacketEvent.Receive>(event -> {
            if (PullCrystal.mc.world == null || PullCrystal.mc.player == null || PullCrystal.mc.player.isDead) {
            }
            else {
                if (event.getPacket() instanceof SPacketSoundEffect) {
                    ...
                    if (packet3.getCategory() == SoundCategory.BLOCKS && packet3.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE) {
                        for (final Entity crystal : new ArrayList<Entity>(PullCrystal.mc.world.loadedEntityList)) {
    ...
    public void onEnable() {
        final BlockPos lastBestPiston = null;
        this.lastBestRedstone = lastBestPiston;
        this.lastBestCrystal = lastBestPiston;
        this.lastBestPiston = lastBestPiston;
        final Vec3d movingPistonNow = new Vec3d(-1.0, -1.0, -1.0);
        this.movingRedstoneNow = movingPistonNow;
        this.movingCrystalNow = movingPistonNow;
        this.movingPistonNow = movingPistonNow;
        this.saveArray = new BlockPos[25];
        this.first = true;
        this.reset();
    }
    ...
    public void doPA() {
        ...
        if (LemonClient.speedUtil.getPlayerSpeed(PullCrystal.mc.player) > this.maxSelfSpeed.getValue()) {
            return;
        }
        this._doPA();
    }
    ...
    public void _doPA() {
        ...
        try {
            ...
            if (this.alwaysCalc.getValue() || this.boom || PullCrystal.target == null || !EntityUtil.isAlive(PullCrystal.target)) {
                final PistonAuraPos pos = this.findSpace();
                if (pos == null) {
                    this.first = true;
                    PullCrystal.target = null;
            ...
            if (this.placedCrystal) {
                ...
                this.placedRedstone = b;
                this.placedPiston = b;
            }
            else {
                final Block piston = BlockUtil.getBlock(this.pistonPos);
            ...
            if (this.breakTimer.passedDms(this.breakDelay.getValue())) {
                this.breakCrystal(this.placedCrystal);
            }
            final float[] angle = MathUtil.calcAngle(new Vec3d(this.targetPos), new Vec3d(this.crystalPos));
    ...
    private void placePiston(final boolean work) {
        ...
        PullCrystal.mc.player.connection.sendPacket(new CPacketPlayer.Rotation(this.rotation.x, this.rotation.y, true));
    ...
    private void placeCrystal(final boolean work) {
        ...
        if (this.crystalBypass.getValue() && (this.crystalSlot >= 9 || this.force.getValue()) && hand == EnumHand.MAIN_HAND) {
            ...
            PullCrystal.mc.player.connection.sendPacket(new CPacketClickWindow(0, slot, PullCrystal.mc.player.inventory.currentItem, ClickType.SWAP, ItemStack.EMPTY, PullCrystal.mc.player.inventoryContainer.getNextTransactionID(PullCrystal.mc.player.inventory)));
            ...
            PullCrystal.mc.player.connection.sendPacket(new CPacketClickWindow(0, slot, PullCrystal.mc.player.inventory.currentItem, ClickType.SWAP, Items.END_CRYSTAL.getDefaultInstance(), PullCrystal.mc.player.inventoryContainer.getNextTransactionID(PullCrystal.mc.player.inventory)));
        }
        else {
        ...
        if (this.placedCrystal) {
            if (this.fire.getValue()) {
                final int slot = BurrowUtil.findHotbarBlock(Items.FLINT_AND_STEEL.getClass());
                if (slot != -1) {
                    this.setItem(slot, false);
                    this.placeBlock(this.firePos = this.crystalPos.up(), this.packetPlace.getValue());
                    if (!this.dance.getValue()) {
                        this.setItem(slot, true);
                    }
                }
            }
            PullCrystal.mc.playerController.onPlayerDamageBlock(this.redStonePos, EnumFacing.UP);
        }
    }
    ...
    private void breakCrystal(final boolean work) {
        if (!work) {
            return;
        }
        if (!this.startBreakTimer.passedDms(this.startBreakDelay.getValue())) {
            return;
        }
        final Entity crystal = (Entity)PullCrystal.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(this.crystalPos.up(2))).stream().filter(e -> e instanceof EntityEnderCrystal).min(Comparator.comparing(e -> this.getDistance(PullCrystal.target, e))).orElse(null);
        if (crystal != null) {
            this.breakTimer.reset();
            final int oldSlot = PullCrystal.mc.player.inventory.currentItem;
            if (this.antiWeakness.getValue() && PullCrystal.mc.player.isPotionActive(MobEffects.WEAKNESS)) {
                int newSlot = -1;
                for (int i = 0; i < 9; ++i) {
                    final ItemStack stack = PullCrystal.mc.player.inventory.getStackInSlot(i);
            ...
            if (this.packetBreak.getValue()) {
                PullCrystal.mc.player.connection.sendPacket(new CPacketUseEntity(crystal));
            }
            else {
                PullCrystal.mc.playerController.attackEntity(PullCrystal.mc.player, crystal);
            }
            if (this.swingArm.getValue()) {
                PullCrystal.mc.player.swingArm(EnumHand.MAIN_HAND);
            }
    ...
    public boolean prepareSpace() {
            ...
            if (this.intersectsWithEntity(this.pistonPos)) {
                ...
                if (this.canPlace(piston) && BlockUtil.canReplace(piston) && !this.isPos2(piston, this.redStonePos)) {
                    if (this.intersectsWithEntity(piston)) {
                        ...
                        BlockUtil.placeBlock(piston, EnumHand.MAIN_HAND, this.rotate.getValue(), this.packetPlace.getValue(), this.strict.getValue(), this.baseRaytrace.getValue(), this.swingArm.getValue());
                    }
                }
                else {
                    this.reset();
        ...
        if ((!this.canPlace(this.redStonePos) || (!this.useBlock && this.redStonePos.getY() == this.pistonPos.getY())) && this.canPlace(redstone) && !this.isPos2(redstone, this.pistonPos)) {
            if (this.intersectsWithEntity(redstone)) {
                ...
                this.setItem(this.obbySlot, false);
                BlockUtil.placeBlock(redstone, EnumHand.MAIN_HAND, this.rotate.getValue(), this.packetPlace.getValue(), this.strict.getValue(), this.baseRaytrace.getValue(), this.swingArm.getValue());
                if (!this.dance.getValue()) {
                    this.setItem(this.obbySlot, true);
                }
            }
    ...
    public PistonAuraPos findSpace() {
        ...
        for (final EntityPlayer target : PlayerUtil.getNearPlayers(this.range.getValue() + 4.0, this.maxTarget.getValue())) {
            ...
            for (final Vec3d vec3d : this.sides) {
                ...
                BlockPos cPos = null;
                for (final Entity entity : PullCrystal.mc.world.loadedEntityList) {
                        ...
                        for (final BlockPos side : offsets) {
                            ...
                            Label_1256: {
                                ...
                                if (cPos == null) {
                                    ...
                                    if (!PullCrystal.mc.world.isAirBlock(crystalPos.up())) {
                                        break Label_1256;
                                    }
                                    if (!PullCrystal.mc.world.isAirBlock(crystalPos.up(2))) {
                                        break Label_1256;
                                    }
                                    if (PlayerUtil.getDistanceI(crystalPos) > this.range.getValue()) {
                                        break Label_1256;
                                    }
                                    if (!PullCrystal.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(crystalPos.up())).isEmpty()) {
                                        break Label_1256;
                                    }
                                    if (!PullCrystal.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(crystalPos.up(2))).isEmpty()) {
                                        break Label_1256;
                                    }
                                }
                                final BlockPos normal = targetPos.add(offset.getX() * -1, y, offset.getZ() * -1);
                                final BlockPos side2 = normal.add(offset.getZ(), 0, offset.getX());
                                final BlockPos side3 = normal.add(offset.getZ() * -1, 0, offset.getX() * -1);
                                final BlockPos side4 = side2.add(offset);
                                final BlockPos side5 = side3.add(offset);
                                final BlockPos side6 = side4.add(offset);
                                final BlockPos side7 = side5.add(offset);
                                final BlockPos side8 = side6.add(offset);
                                final BlockPos side9 = side7.add(offset);
                                final BlockPos side10 = crystalPos.add(offset);
                                ...
                                if (!this.fire.getValue() || BurrowUtil.findHotbarBlock(Items.FLINT_AND_STEEL.getClass()) == -1) {
                                    ...
                                    this.add(pistons, side9);
                                    this.add(pistons, side10);
                                }
                                pistons.removeIf(p -> {
                                ...
                                if (!pistons.isEmpty()) {
                                    final List<BlockPos> pistonList = pistons.stream().filter(p -> {
                                        if (this.fiveB.getValue() && BlockUtil.getBlock(p.add(finalOffset.getX() * -1, 0, finalOffset.getZ() * -1)) == Blocks.BEDROCK) {
                                            return false;
                                        }
                                        else {
                                            final BlockPos headPos = p.add(finalOffset);
                                            if (ColorMain.INSTANCE.breakList.contains(headPos) || ColorMain.INSTANCE.breakList.contains(p)) {
                                                ...
                                                if (headBlock == Blocks.BEDROCK || headBlock == Blocks.OBSIDIAN || headBlock == Blocks.ENDER_CHEST || this.checkPos(headPos)) {
                                                    return false;
                                                }
                                                else {
                                                    final boolean isPiston = BlockUtil.getBlock(p) instanceof BlockPistonBase;
                                                    if (!isPiston) {
                                                        if (!this.canPlace(p)) {
                                                            return false;
                                                        }
                                                        else if (PullCrystal.mc.player.getDistance(p.getX() + 0.5, p.getY() + 0.5, p.getZ() + 0.5) > this.range.getValue()) {
                                                            return false;
                                                        }
                                                        else {
                                                            final double feetY = PullCrystal.mc.player.posY;
                                                            if ((PlayerUtil.getDistanceI(p) < 0.8 + p.getY() - feetY && p.getY() > feetY + 1.0) || (PlayerUtil.getDistanceI(p) < 1.8 + feetY - p.getY() && p.getY() < feetY)) {
                                                                return false;
                                                            }
                                                        }
                                                    }
                                                    ...
                                                    final BlockPos redstonePos = this.getRedStonePos(crystalPos, p, finalOffset);
                                                    if (redstonePos == null) {
                                                        return false;
                                                    }
                                                    else {
                                                        ...
                                                        if (!isPiston) {
                                                            if (!BlockUtil.canPlaceWithoutBase(p, this.strict.getValue(), this.needRaytrace(p), ((this.base.getValue() || this.canPlace(redstonePos.down())) && this.obbySlot != -1) || this.canPlace(redstonePos) || BlockUtil.getBlock(p) instanceof BlockPistonBase)) {
                                                                return b;
                                                            }
                                                        }
                                                        return b;
                ...
                final List<PistonAuraPos> paList = can.stream().filter(p -> !p.block || p.offset.z == 1).collect(Collectors.toList());
                if (paList.isEmpty()) {
                    paList.addAll(can);
                }
    ...
    public boolean isFacing(final BlockPos pos, final BlockPos facingPos) {
        final ImmutableMap<IProperty<?>, Comparable<?>> properties = PullCrystal.mc.world.getBlockState(pos).getProperties();
    ...
    public BlockPos getRedStonePos(final BlockPos crystalPos, final BlockPos pistonPos, final BlockPos offset) {
        ...
        final BlockPos pistonPush = pistonPos.add(offset.getX(), 0, offset.getZ());
        if (this.useBlock) {
        ...
        return redstone.stream().filter(p -> !ColorMain.INSTANCE.breakList.contains(p) && (p.getX() != crystalPos.getX() || p.getZ() != crystalPos.getZ()) && (p.getX() != pistonPush.getX() || p.getZ() != pistonPush.getZ()) && PullCrystal.mc.player.getDistance(p.getX() + 0.5, p.getY() + 0.5, p.getZ() + 0.5) <= this.range.getValue() && BlockUtil.canPlaceWithoutBase(p, this.strict.getValue(), this.needRaytrace(p), this.base.getValue())).min(Comparator.comparingInt(this::blockLevel)).orElse(null);
    }
    ...
    private int getItemHotbar() {
        for (int i = 0; i < (this.crystalBypass.getValue() ? 36 : 9); ++i) {
            final Item item = PullCrystal.mc.player.inventory.getStackInSlot(i).getItem();
    ...
    public boolean findMaterials() {
        ...
        if (PullCrystal.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL) {
    ...
    public void setItem(final int slot, final boolean back) {
        if (slot == 999) {
            return;
        }
    ...
    private void normalSwitch(final int slot) {
        if (this.packetSwitch.getValue()) {
            PullCrystal.mc.player.connection.sendPacket(new CPacketHeldItemChange(slot));
        }
        else {
            PullCrystal.mc.player.inventory.currentItem = slot;
        }
    }
    
    private void bypassSwitch(final int slot) {
        PullCrystal.mc.playerController.windowClick(0, slot + 36, PullCrystal.mc.player.inventory.currentItem, ClickType.SWAP, PullCrystal.mc.player);
    }
    
    public void restoreItem() {
        if (this.silentSwitch.getValue() && !this.bypass.getValue()) {
            if (this.packetSwitch.getValue()) {
                PullCrystal.mc.player.connection.sendPacket(new CPacketHeldItemChange(this.oldSlot));
            }
            else {
                PullCrystal.mc.player.inventory.currentItem = this.oldSlot;
                PullCrystal.mc.playerController.updateController();
            }
        }
    }
    ...
    private boolean placeBlock(final BlockPos pos, final boolean packet) {
        ...
        if (!ColorMain.INSTANCE.sneaking && BlockUtil.blackList.contains(BlockUtil.getBlock(neighbour))) {
            PullCrystal.mc.player.connection.sendPacket(new CPacketEntityAction(PullCrystal.mc.player, CPacketEntityAction.Action.START_SNEAKING));
            PullCrystal.mc.player.setSneaking(true);
            sneak = true;
        }
        if (packet) {
            ...
            PullCrystal.mc.playerController.processRightClickBlock(PullCrystal.mc.player, PullCrystal.mc.world, neighbour, opposite, hitVec, EnumHand.MAIN_HAND);
        }
        if (this.swingArm.getValue()) {
            PullCrystal.mc.player.swingArm(EnumHand.MAIN_HAND);
        }
        ...
        if (sneak) {
            PullCrystal.mc.player.connection.sendPacket(new CPacketEntityAction(PullCrystal.mc.player, CPacketEntityAction.Action.STOP_SNEAKING));
            PullCrystal.mc.player.setSneaking(false);
        }
        return true;
    }
    ...
    public static void rightClickBlock(final BlockPos pos, final Vec3d vec, final EnumHand hand, final EnumFacing direction) {
        ...
        PullCrystal.mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(pos, direction, hand, f, f2, f3));
    }
    ...
    private boolean needRaytrace(final BlockPos pos) {
        return PullCrystal.mc.player.getDistance(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5) > this.forceRange.getValue() && this.raytrace.getValue();
    }
    
    @Override
    public void onWorldRender(final RenderEvent event) {
        if (PullCrystal.mc.world == null || PullCrystal.mc.player == null) {
            return;
        }
            ...
            if (this.iq.getValue()) {
                if (this.lastBestPiston != null) {
                    if (this.movingPistonNow.x == -1.0 && this.movingPistonNow.y == -1.0 && this.movingPistonNow.z == -1.0) {
                        this.movingPistonNow = new Vec3d((float)this.lastBestPiston.getX(), (float)this.lastBestPiston.getY(), (float)this.lastBestPiston.getZ());
                    }
                    this.movingPistonNow = new Vec3d(this.movingPistonNow.x + (this.lastBestPiston.getX() - this.movingPistonNow.x) * this.speed.getValue().floatValue(), this.movingPistonNow.y + (this.lastBestPiston.getY() - this.movingPistonNow.y) * this.speed.getValue().floatValue(), this.movingPistonNow.z + (this.lastBestPiston.getZ() - this.movingPistonNow.z) * this.speed.getValue().floatValue());
                    this.drawBoxMain(this.movingPistonNow.x, this.movingPistonNow.y, this.movingPistonNow.z, 255, 255, 150);
                    if (Math.abs(this.movingPistonNow.x - this.lastBestPiston.getX()) <= 0.125 && Math.abs(this.movingPistonNow.y - this.lastBestPiston.getY()) <= 0.125 && Math.abs(this.movingPistonNow.z - this.lastBestPiston.getZ()) <= 0.125) {
                        this.lastBestPiston = null;
                    }
                }
                if (this.lastBestCrystal != null) {
                    if (this.movingCrystalNow.x == -1.0 && this.movingCrystalNow.y == -1.0 && this.movingCrystalNow.z == -1.0) {
                        this.movingCrystalNow = new Vec3d((float)this.lastBestCrystal.getX(), (float)this.lastBestCrystal.getY(), (float)this.lastBestCrystal.getZ());
                    }
                    this.movingCrystalNow = new Vec3d(this.movingCrystalNow.x + (this.lastBestCrystal.getX() - this.movingCrystalNow.x) * this.speed.getValue().floatValue(), this.movingCrystalNow.y + (this.lastBestCrystal.getY() - this.movingCrystalNow.y) * this.speed.getValue().floatValue(), this.movingCrystalNow.z + (this.lastBestCrystal.getZ() - this.movingCrystalNow.z) * this.speed.getValue().floatValue());
                    this.drawBoxMain(this.movingCrystalNow.x, this.movingCrystalNow.y, this.movingCrystalNow.z, 255, 255, 255);
                    if (Math.abs(this.movingCrystalNow.x - this.lastBestCrystal.getX()) <= 0.125 && Math.abs(this.movingCrystalNow.y - this.lastBestCrystal.getY()) <= 0.125 && Math.abs(this.movingCrystalNow.z - this.lastBestCrystal.getZ()) <= 0.125) {
                        this.lastBestCrystal = null;
                    }
                }
                if (this.lastBestRedstone != null) {
                    if (this.movingRedstoneNow.x == -1.0 && this.movingRedstoneNow.y == -1.0 && this.movingRedstoneNow.z == -1.0) {
                        this.movingRedstoneNow = new Vec3d((float)this.lastBestRedstone.getX(), (float)this.lastBestRedstone.getY(), (float)this.lastBestRedstone.getZ());
                    }
                    this.movingRedstoneNow = new Vec3d(this.movingRedstoneNow.x + (this.lastBestRedstone.getX() - this.movingRedstoneNow.x) * this.speed.getValue().floatValue(), this.movingRedstoneNow.y + (this.lastBestRedstone.getY() - this.movingRedstoneNow.y) * this.speed.getValue().floatValue(), this.movingRedstoneNow.z + (this.lastBestRedstone.getZ() - this.movingRedstoneNow.z) * this.speed.getValue().floatValue());
                    this.drawBoxMain(this.movingRedstoneNow.x, this.movingRedstoneNow.y, this.movingRedstoneNow.z, 225, 50, 50);
                    if (Math.abs(this.movingRedstoneNow.x - this.lastBestRedstone.getX()) <= 0.125 && Math.abs(this.movingRedstoneNow.y - this.lastBestRedstone.getY()) <= 0.125 && Math.abs(this.movingRedstoneNow.z - this.lastBestRedstone.getZ()) <= 0.125) {
                        this.lastBestRedstone = null;
                    }
                }
            }
    ...
    public String getHudInfo() {
        return (this.hud.getValue() && PullCrystal.target != null) ? ("[" + ChatFormatting.WHITE + PullCrystal.target.getName() + ChatFormatting.GRAY + "]") : "";
    }
