...
import com.lemonclient.api.util.misc.TriPredicate;
...
public class RayTracer
{
    ...
    public static RayTraceResult rayTraceEntities(final World world, final Entity from, final double range, final EntityPlayer player, final Predicate<Entity> entityCheck, final Entity... additional) {
        final Vec2f vec = PlayerPacketManager.INSTANCE.getServerSideRotation();
        return rayTraceEntities(world, from, range, player.posX, player.posY, player.posZ, vec.x, vec.y, player.getEntityBoundingBox(), entityCheck, additional);
    }
    ...
    public static RayTraceResult rayTraceEntities(final World world, final Entity from, final double range, final double posX, final double posY, final double posZ, final float yaw, final float pitch, final AxisAlignedBB fromBB, final Predicate<Entity> entityCheck, final Entity... additional) {
        ...
        if (RayTracer.mc.isCallingFromMinecraftThread()) {
            entities = world.getEntitiesInAABBexcluding(from, within, predicate);
        }
        else {
            entities = RayTracer.mc.world.loadedEntityList.stream().filter(e -> e != null && e.getEntityBoundingBox().intersects(within) && predicate.test(e)).collect(Collectors.toList());
        }
        ...
        for (final Entity entity2 : entities) {
            final AxisAlignedBB bb = entity2.getEntityBoundingBox().grow(entity2.getCollisionBorderSize());
    ...
    public static RayTraceResult trace(final World world, final Vec3d start, final Vec3d end, final boolean stopOnLiquid, final boolean ignoreBlockWithoutBoundingBox, final boolean returnLastUncollidableBlock, final BiPredicate<Block, BlockPos> blockChecker) {
        return trace(world, world, start, end, stopOnLiquid, ignoreBlockWithoutBoundingBox, returnLastUncollidableBlock, blockChecker);
    }
    ...
    static {
        ...
        RayTracer.mc = Minecraft.getMinecraft();
    }
}
