...
public class RayTraceFactory
{
    ...
    public static Ray fullTrace(final Entity from, final World world, final BlockPos pos, final double resolution) {
        Ray dumbRay = null;
        ...
        for (final EnumFacing facing : getOptimalFacings(from, pos)) {
            ...
            if (!state.getMaterial().isReplaceable()) {
                ...
                final double dist = from.getDistanceSq(offset.x + 0.5, offset.y + 0.5, offset.z + 0.5);
    ...
    public static Ray rayTrace(final Entity from, final BlockPos on, final EnumFacing facing, final World access, final IBlockState state, final double res) {
        final Vec3d start = new Vec3d(from.posX, from.posY + from.getEyeHeight(), from.posZ);
        ...
        if (res >= 1.0) {
            ...
            final double d = RayTraceFactory.mc.playerController.getBlockReachDistance();
            final Vec3d rotations = start.add(look.x * d, look.y * d, look.z * d);
            final RayTraceResult result = RayTracer.trace(RayTraceFactory.mc.world, access, start, rotations, false, false, true);
            ...
            if (res <= 0.0) {
                ...
                for (final Vec3d vec : vectors) {
                    final RayTraceResult ray = RayTracer.trace(RayTraceFactory.mc.world, access, start, vec, false, false, true);
            ...
            for (double x2 = Math.min(minX, maxX); x2 <= endX; x2 += res) {
                for (double y2 = Math.min(minY, maxY); y2 <= endY; y2 += res) {
                    for (double z2 = Math.min(minZ, maxZ); z2 <= endZ; z2 += res) {
                        ...
                        final RayTraceResult ray2 = RayTracer.trace(RayTraceFactory.mc.world, access, start, vector, false, false, true);
                        if (ray2 != null && facing == ray2.sideHit && on.equals(ray2.getBlockPos())) {
    ...
    private static float[] rots(final BlockPos pos, final EnumFacing facing, final Entity from, final World world, final IBlockState state) {
    ...
    static {
        ...
        RayTraceFactory.mc = Minecraft.getMinecraft();
    }
}
